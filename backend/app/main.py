...
def build_prompt_m10(result: str) -> str:
    return f"""【重要】你只能输出纯JSON格式，不要输出任何其他文字。

你是一位民事检察监督专家。请分析以下判决书片段，判断是否存在"有金钱给付义务但未载明加倍支付迟延履行利息"的问题。
...
判断标准：
1. 判断裁判结果是否包含金钱给付义务：
   - 包含"给付"、"支付"、"赔偿"、"返还"、"退还"、"补偿"等词，且明确写明了金额或计算方式
   - 精神损害赔偿的"精神抚慰金"不属于金钱给付义务范畴
   - 但以下费用属于金钱给付义务：被告应支付给原告的公告费、保全费、鉴定费等程序性费用（注意：案件受理费通常交至法院账户，不属于被告向原告的金钱给付），以及合同违约金
2. 如果有金钱给付义务，检查是否同时满足以下任一条件：
   A. 明确写了"加倍支付迟延履行期间的利息/迟延履行金"
   B. 引用了《民事诉讼法》第253条或《民诉法解释》第506条
   C. 写了"逾期按日万分之五（或法定利率）支付利息"
   D. 写了"自应当给付之日起参照LPR利率加倍支付利息"
3. 合规情形（以下情况不属于问题）：
   - 判决仅要求"继续履行"、"恢复原状"、"排除妨害"等行为义务
   - 金钱给付已明确"即时履行"且无履行期限
   - 生效判决的再审案件已明确利息计算方式
4. 风险等级：
   - 高：有金钱给付、有履行期限、但无任何利息规定
   - 中：有金钱给付、无明确期限、也无利息规定
   - 低：金额极小或双方均有过错情形

【示例1 - 有问题】
裁判结果：被告应于判决生效后十日内给付原告货款人民币80000元。
输出：{{"has_issue": true, "reason": "被告应于判决生效后十日内给付原告货款80000元，但未载明加倍支付迟延履行利息", "risk_level": "高", "suggestion": "建议补充逾期付款加倍支付利息条款"}}

【示例2 - 有问题】
裁判结果：一、被告赔偿原告经济损失200000元，于判决生效后三十日内支付；二、驳回原告其他诉讼请求。
输出：{{"has_issue": true, "reason": "被告赔偿原告经济损失200000元，但未载明逾期支付的利息规定", "risk_level": "高", "suggestion": "建议参照民事诉讼法第253条补充利息条款"}}

【示例3 - 无问题】
裁判结果：被告应于判决生效后十五日内返还原告货款50000元，并加倍支付迟延履行期间的利息。
输出：{{"has_issue": false, "reason": "裁判结果明确载明加倍支付迟延履行期间的利息", "risk_level": "低", "suggestion": ""}}

【示例4 - 无问题】
裁判结果：一、被告于判决生效后二十日内给付原告借款本金100000元及利息（自2023年1月1日起按年利率15%计算至实际清偿之日）；二、如被告逾期支付，加倍支付迟延履行利息。
输出：{{"has_issue": false, "reason": "裁判结果已明确利息计算方式及加倍支付迟延履行利息条款", "risk_level": "低", "suggestion": ""}}

【示例5 - 无问题（非金钱给付）】
裁判结果：一、被告应于判决生效后三十日内将涉案房屋腾空并返还原告；二、驳回原告其他诉讼请求。
输出：{{"has_issue": false, "reason": "裁判结果系行为义务（腾空返还房屋），不涉及金钱给付", "risk_level": "低", "suggestion": ""}}

【示例6 - 无问题（引用法条）】
裁判结果：被告应于判决生效后十日内支付原告工程款300000元，逾期支付的，依据《民事诉讼法》第253条加倍支付迟延履行期间的利息。
输出：{{"has_issue": false, "reason": "裁判结果明确引用《民事诉讼法》第253条加倍支付迟延履行利息", "risk_level": "低", "suggestion": ""}}
